...
def verify_mcq_flow():
    with sync_playwright() as p:
        ...
        # Answer first 5 questions only (to save time and avoid misclicks)
        for i in range(5):
            page.locator("button >> text=A").nth(i).click()
            
        # We need to answer at least one to enable submit?
        # Yes.
        
        # Click Next Page to go to Batch 2
        page.get_by_text("Next Page").click()
        
        # Verify Batch 2
        page.wait_for_selector("text=Batch 2", timeout=10000)
        page.screenshot(path="verification/mcq_batch_page2_verified.png")
        
        # On Batch 2, "Submit Final" should be visible (if hasMore is false)
        # 55 items. Batch 2 is 51-55. hasMore should be false.
        ...
        page.wait_for_selector("text=Explanation:", timeout=10000)
